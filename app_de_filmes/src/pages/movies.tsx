...
export const Movies: React.FC = () => {
  ...
  // Busca filmes filtrados
  const { data, isLoading, isError } = useQuery({
    queryKey: ['discover', 'movie', genreId, year, minRating, sortBy, page],
    queryFn: () =>
      discoverMedia({
        type: 'movie',
        genreId,
  ...
  const totalPages = data?.total_pages ? Math.min(data.total_pages, 500) : 1; // Limite de 500 páginas do TMDB
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Encontre o filme perfeito utilizando os filtros inteligentes.
            </p>
          </div>
        ...
        {isError ? (
          <div className="text-center py-12">
            <p className="text-red-500 font-medium">Ocorreu um erro ao carregar os filmes. Tente novamente mais tarde.</p>
          </div>
        ) : isLoading ? (
          ...
          <div className="text-center py-16">
            <p className="text-gray-500 dark:text-gray-400 text-lg">Nenhum filme corresponde aos filtros selecionados.</p>
          </div>
        ) : (
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
